...
1. **Vite plugin** (`autoi18nPlugin.ts`) — build-time. `enforce: 'pre'` so it sees SFC source before `@vitejs/plugin-vue` compiles it. In `transform()` it:
   ...
   - **zero-markup scan (v0.2.0, `scan/` directory)**: independent of the explicit path, parses the whole SFC (`@vue/compiler-sfc`) and scans CJK-bearing string literals not inside explicit calls — template text nodes / interpolation / bound-attribute expressions (`templateScan.ts` + `@babel/parser` `parseExpression` on the expression fragment) and script literals (`scriptScan.ts`). A hit must contain CJK (`scan/cjk.ts`); object keys, computed member access (`obj['中文key']`), import paths and comments are excluded via parent-node checks (`babelCollect.ts`). Rewrites happen via `magic-string` (`rewrite.ts`) into `_autoScanTranslate(...)` calls (single-quoted literals — double quotes would break HTML attribute delimiters) plus an inject block importing the exported `autoi18nInfo` reactive state (NO `inject()` — works in plain `<script>` top level; identifiers all `_`-prefixed so both inject blocks can coexist in one module). **Zero-markup injection runs in dev AND production** (unlike the explicit path, which keeps its pre-0.2.0 dev-only behavior); SFC parse failures and sub-requests (`id` containing `?`) safely skip the scan. `autoScan: false` disables the whole scan; `exclude` (string includes / RegExp test on the module path) skips matched files.
...
Template hits are only rewritten when the injected helper is visible to template expressions: `<script setup>` (or template-only SFCs, whose inject block lands in an appended setup block) — plain `<script>` (Options API) components skip template hits conservatively (`_ctx.*` cannot see module-level bindings; use the explicit global `$translate` there) while script literals are still translated.
Zero-markup reactive boundary: template texts/expressions rewritten by the scan are reactive on locale switch, but **top-level script literals are evaluated once at setup** — texts that must follow locale switching belong inside `computed` (the scan still rewrites them there). Ignore markers: `/* autoi18n-ignore */` covers the statement right after it (script), `<!-- autoi18n-ignore -->` covers the element right after it (template). Interpolated template literals (`` `共${n}条` ``) and static attributes are skipped in v0.2.0.
